  ...
  prisma: {
    ...
    $transaction: jest.fn(async (args) => {
      if (Array.isArray(args)) {
        ...
        try {
          const results = await Promise.all(operations.map(op => typeof op === 'function' ? op(prisma) : op));
          return results;
        } catch (error) {
      ...
      if (typeof operations === 'function') { // Corrected 'args' to 'operations'
        try {
...
// 테스트 데이터
const mockProduct: Product = { // Added Product type for better checking
  id: 'test-product-id',
  name: '테스트 상품',
  sku: 'TEST001',
  category: '테스트',
...
describe('Product Service', () => {
  ...
  describe('productService.moveLocation (simulating moveProductLocation)', () => {
    ...
    beforeEach(() => {
      ...
      (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue({ ...mockProduct, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) });
      ...
      (prisma.$transaction as jest.Mock<any, any>).mockImplementation(async (callback) => {
        // This mock simulates Prisma's $transaction by invoking the callback
        // with the already mocked 'prisma' client.
        // Errors thrown within the callback will naturally reject the promise.
        try {
    ...
    it('제품 위치를 성공적으로 변경해야 함', async () => {
      ...
      const result = await productService.moveLocation(mockProduct.id, mockUser.id, '창고 B-2');
    ...
    it('제품이 존재하지 않을 경우 에러를 발생시켜야 함', async () => {
      (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValueOnce(null);
      await expect(
    ...
    it('현재 위치와 동일한 위치로 변경 시 에러를 발생시켜야 함', async () => {
      ...
      const prismaMockProduct = { ...mockProduct, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) };
      (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(prismaMockProduct);
      await expect(
        productService.moveLocation(mockProduct.id, mockUser.id, mockProduct.location),
  ...
  describe('productService.getProductById', () => {
    it('should return a product if found', async () => {
      ...
      const prismaMockReturnValue = { // Data Prisma would return (Date objects)
        ...specificMockProduct,
      ...
      (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValueOnce(prismaMockReturnValue);
    ...
    it('should throw an error if product is not found', async () => {
      (prisma.product.findUnique as jest.Mock<any, any>).mockImplementation(async (args) => {
        if (args.where.id === 'not-found-id') return Promise.resolve(null);
        // Fallback for other IDs if any, though test is specific
        return Promise.resolve({ ...mockProduct, id: args.where.id, createdAt: new Date(), updatedAt: new Date() });
      });

      await expect(productService.getProductById('not-found-id')).rejects.toThrow(
        'Product not found',
      );
      expect(prisma.product.findUnique).toHaveBeenCalledWith({
  ...
  describe('productService.createProduct', () => {
    const newProductData: ProductFormData = { // Changed from CreateProductData to ProductFormData
    ...
    it('should create and return a new product if SKU does not exist', async () => {
      (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(null); // SKU check
...
describe('productService.getProducts', () => {
  it('should return all products if no filters are provided', async () => {
    const mockProductsFromPrisma = [
      { ...mockProduct, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) },
      { ...mockProduct, id: 'prod2', sku: 'SKU002', createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) },
    ];
    (prisma.product.findMany as jest.Mock<any, any>).mockResolvedValue(mockProductsFromPrisma);
  ...
  it('should filter by search term for name or SKU', async () => {
    ...
    const mockFilteredFromPrisma = [{ ...mockProduct, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) }];
    (prisma.product.findMany as jest.Mock<any, any>).mockResolvedValue(mockFilteredFromPrisma);
  ...
  it('should handle a combination of filters', async () => {
    ...
    const mockFilteredFromPrisma = [{ ...mockProduct, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) }];
    (prisma.product.findMany as jest.Mock<any, any>).mockResolvedValue(mockFilteredFromPrisma);
    await productService.getProducts({ search: searchTerm, category: category });
    expect(prisma.product.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          OR: [
...
describe('productService.adjustInventory', () => {
  ...
  it('should adjust inventory and return product and adjustment history', async () => {
    const initialProductPrisma = { // Data from Prisma (Date objects)
      ...mockProduct,
      id: productId,
      quantity: 10,
      safetyStock: 5,
    ...
    const expectedNewQuantity = initialProductPrisma.quantity + adjustmentQuantity;

    const updatedProductFromPrisma = { // Prisma update result (Date objects)
      ...initialProductPrisma,
      quantity: expectedNewQuantity,
      updatedAt: new Date(),
    };
    const expectedProductData: Product = { // What the service should return (ISO strings)
        ...initialProductPrisma,
    ...
    const adjustmentRecordFromPrisma = { // Prisma create result (Date objects)
      id: 'adjHistId',
      productId,
      userId: mockUser.id,
     ...
     const expectedAdjustmentRecord: InventoryAdjustment = { // What the service should return
        ...adjustmentRecordFromPrisma,
        createdAt: adjustmentRecordFromPrisma.createdAt.toISOString(),
    };

    (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(initialProductPrisma);
    (prisma.product.update as jest.Mock<any, any>).mockResolvedValue(updatedProductFromPrisma);
    (prisma.inventoryAdjustment.create as jest.Mock<any, any>).mockResolvedValue(
      adjustmentRecordFromPrisma,
    );
    (prisma.$transaction as jest.Mock<any, any>).mockImplementation(async (callback) => {
      try {
    ...
    const result = await productService.adjustInventory(productId, mockUser.id, adjustmentQuantity, memo);
    ...
    expect(prisma.inventoryAdjustment.create).toHaveBeenCalledWith({
      data: { productId, userId: mockUser.id, quantity: adjustmentQuantity, memo },
    });
  ...
  it('should trigger low stock alert if quantity falls to or below safety stock', async () => {
    const initialProductPrismaLowStock = { // Data from Prisma (Date objects)
      ...mockProduct,
      id: 'lowStockProd',
      quantity: 10,
      safetyStock: 10,
    ...
    const updatedProductFromPrismaLowStock = { // Prisma update result (Date objects)
      ...initialProductPrismaLowStock,
      quantity: expectedNewQuantity,
      updatedAt: new Date(),
    };
    const adjustmentRecordFromPrismaLowStock = { // Prisma create result (Date objects)
      id: 'adjHistId2',
    ...
    (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(initialProductPrismaLowStock);
    (prisma.product.update as jest.Mock<any, any>).mockResolvedValue(updatedProductFromPrismaLowStock);
    (prisma.inventoryAdjustment.create as jest.Mock<any, any>).mockResolvedValue(
      adjustmentRecordFromPrismaLowStock,
    );
    ...
    await productService.adjustInventory('lowStockProd', mockUser.id, adjustmentQuantityForLowStock, memoReducingStock);
  ...
  it('should throw error if adjustment results in negative stock', async () => {
    const initialProductPrismaNegStock = { ...mockProduct, id: 'negStockProd', quantity: 5, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) };
    (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(initialProductPrismaNegStock);
    const adjustmentQuantityNeg = -10;
    const memoNegStock = 'Attempt negative stock';
    await expect(productService.adjustInventory('negStockProd', mockUser.id, adjustmentQuantityNeg, memoNegStock)).rejects.toThrow(
      '재고가 부족합니다.',
    );
  });
  ...
  it('should throw error if product for adjustment is not found', async () => {
    ...
    await expect(productService.adjustInventory('non-exist-adj', mockUser.id, adjustmentQuantityNotFound, memoNotFound)).rejects.toThrow(
      '제품을 찾을 수 없습니다.',
    );
  });
});
...
describe('productService.updateProduct', () => {
  ...
  const updateData: Partial<ProductFormData> = { // Changed UpdateProductData to ProductFormData
  ...
  it('should update and return the product if found', async () => {
    const existingProductPrisma = { ...mockProduct, id: productIdToUpdate, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) };
    (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(existingProductPrisma);

    const updatedDate = new Date();
    const updatedProductFromPrisma = { // Result from Prisma (Date objects)
      ...existingProductPrisma,
      ...updateData,
      updatedAt: updatedDate,
    };
    const expectedUpdatedProduct: Product = { // Expected by test (ISO strings)
        ...existingProductPrisma,
        ...updateData,
    ...
    const product = await productService.updateProduct(productIdToUpdate, updateData as ProductFormData);
  ...
  it('should throw an error if product to update is not found', async () => {
    ...
    (prisma.product.findUnique as jest.Mock<any, any>).mockImplementationOnce(async (args) => {
    ...
    await expect(
      productService.updateProduct('non-existent-id', updateData as ProductFormData),
...
describe('productService.deleteProduct', () => {
  ...
  it('should delete the product and its related history if found', async () => {
    const existingProductPrisma = { ...mockProduct, id: productIdToDelete, createdAt: new Date(mockProduct.createdAt), updatedAt: new Date(mockProduct.updatedAt) };
    (prisma.product.findUnique as jest.Mock<any, any>).mockResolvedValue(existingProductPrisma);

    (prisma.$transaction as jest.Mock<any, any>).mockImplementation(
      async (operations: any[]) => { // Added type
