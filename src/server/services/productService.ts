...
export const productService = {
  ...
  async adjustInventory(
    ...
    try {
      const response = await apiClient.post<{
        data: { product: Product; adjustment: InventoryAdjustment };
      }>(`/products/${id}/adjust`, data);
      return response.data.data;
    } catch (error) {
  ...
  async moveLocation(
    id: string,
    data: LocationMoveFormData,
  ): Promise<{ product: Product; locationHistory: LocationHistory }> {
    try {
      const response = await apiClient.post<{
        data: { product: Product; locationHistory: LocationHistory };
      }>(`/products/${id}/move`, data);
      return response.data.data;
    } catch (error) {
  ...
  async getLocationHistory(id: string): Promise<LocationHistory[]> {
    try {
      const response = await apiClient.get<{ data: LocationHistory[] }>(
